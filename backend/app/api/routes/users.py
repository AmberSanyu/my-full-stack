...
from app.users import crud  # 引入上面新抽离的用户专属数据访问层
...
def create_user(*, session: SessionDep, user_in: UserCreate) -> Any:
    # 路由层控制：唯一性业务校验
    existing_user = global_crud.get_user_by_email(session=session, email=user_in.email)
    if existing_user:
        raise HTTPException(
            status_code=400,
...
def update_password_me(*, session: SessionDep, body: UpdatePassword, current_user: CurrentUser) -> Any:
    ...
    crud.update_user_password(session=session, db_user=current_user, new_password=body.new_password)
